...
public class ComplexSceneView
extends DrawObjectView
implements Printable, AdjustmentListener
{
...
public
ComplexSceneView(String fileName, JFrame parent, boolean suppressAlert)
{
	...
	try
	{
		...
		complexPickPanel.addMouseListener(new MouseAdapter()
		{
			...
			mousePressed(MouseEvent event)
			{
				Point2D imgCenteredPt = new Point2D.Double(
					(double)event.getX() -
						(double)complexPickPanel.getWidth()/2.0,
					(double)complexPickPanel.getHeight()/2.0 -
						(double)event.getY());
		...
		ioTabPanel = new ComplexSceneIOTab(
			guiBGColor, controlPanelW, controlPanelH);
		...
		annotateTabPanel = new ComplexSceneAnnotateTab(
			guiBGColor, controlPanelW, controlPanelH);
		...
		editTabPanel = new ComplexSceneEditTab(
			guiBGColor, controlPanelW, controlPanelH);
		...
		formatSStrTabPanel = new ComplexSceneFormatSStrTab(
			guiBGColor, controlPanelW, controlPanelH);
...
buildScalePanel()
{
	...
	scale_Panel.setBorder(new TitledBorder(new BevelBorder(0),
		"Scale Figure:"));
		...
		actionPerformed(ActionEvent ae)
		{
			...
			try
			{
				...
				centerWindowToImgCoords(
					imgSpaceX(((double)(getWindowViewX() +
					(getWindowViewW()/2)) - (double)imgWidth/2.0) * scaleVal),
					imgSpaceY(-((double)imgHeight/2.0 - (double)(getWindowViewY() +
					(getWindowViewH()/2))) * scaleVal));
				
				setCursor(complexDefaultCursor);
				setCursor(
					complexDefaultCursor);
				scaleCB.setCursor(
					complexDefaultCursor);

				/* SAVE to show how go from one img to another:
				int imgWidth = complexPickPanel.getWidth();
				int imgHeight = complexPickPanel.getHeight();

				Point2D imgCenteredPt = new Point2D.Double(
					(double)event.getX() - (double)imgWidth/2.0,
					(double)imgHeight/2.0 - (double)event.getY());

				// double scaleVal = getFigureScale()/getComplexPickScaleVal();
				// double scaleVal = getFigureScale()/getComplexPickScaleVal()/getFigureScale();
				// scaleVal /= getFigureScale();
				double scaleVal = 1.0/getComplexPickScaleVal();

				debug("NEW SCALE: " + scaleVal);

				Point2D sceneCenteredPt = new Point2D.Double(
					imgCenteredPt.getX() * scaleVal,
					imgCenteredPt.getY() * scaleVal);
				
				int sceneImgX = imgSpaceX(sceneCenteredPt.getX());
				int sceneImgY = imgSpaceY(-sceneCenteredPt.getY());

				printConsole("pick x,y: " +
					event.getX() + " " + event.getY() + " imgcenteredPt: " + 
					StringUtil.roundStrVal(imgCenteredPt.getX(), 2) + " " +
					StringUtil.roundStrVal(imgCenteredPt.getY(), 2) +
					" sceneCenteredPt: " +
					StringUtil.roundStrVal(sceneCenteredPt.getX(), 2) + " " +
					StringUtil.roundStrVal(sceneCenteredPt.getY(), 2) +
					" sceneimgPt: " + sceneImgX + " " + sceneImgY);
...
buildUndoPanels()
{
	...
	undo_Edit_Bt.setMargin(new Insets(btInsets.top, lftInset,
	...
	undo_Format_Bt.setMargin(new Insets(btInsets.top, lftInset,
		btInsets.bottom, btInsets.right + btLengthAdjust));
...
public
ComplexSceneView(String fileName, JFrame parent)
{
	this (fileName, parent, false);
}

public
ComplexSceneView(String fileName)
{
	this(fileName, null);
}

public
ComplexSceneView(String fileName, boolean testOnly)
{
	try
	{
		this.setInputFileName(fileName);
...
public
ComplexSceneView(ComplexScene2D complexScene)
{
	this(null, null);
...
public
ComplexSceneView(ComplexScene2D complexScene, boolean testOnly)
{
	this.runTestOnly = testOnly;
	this.setComplexScene(complexScene);
}
...
stopXRNA()
{
	...
	{
		if (this.getCurrentInputFile().exists() &&
			!this.getCurrentInputFile().canWrite())
			return (result);
	}
	else
	{
	...
	try
	{
		...
		message[2] = new JLabel("Quitting before writing out will cause loss of any changes made to: " + getComplexScene().getName());
		...
		result = JOptionPane.showOptionDialog(
			frameParent,  // the parent that the dialog blocks
...
stopXRNA_AL = new ActionListener()
{
	public void
	actionPerformed(ActionEvent evt)
	{
...
buildGui()
{
	...
	// this.setBounds(getGuiStartX(), getGuiStartY(), getGuiWidth(), getGuiHeight());
		...
		actionPerformed(ActionEvent evt)
		{
			...
			try
			{
				if (pickStrandPanel == null)
				{
					...
					pickStrandPanel.setPreferredSize(
						new Dimension(200, 300));

					getComplexScene().resetComplexNodes();
					TreeNode complexSceneNode =
						getComplexScene().createComplexNodes();
					...
					pickStrandTree.getSelectionModel().setSelectionMode
				...
				updateBasicInternalFrame("Structure List",
					"Select Current Structure:",
					pickStrandPanel, pickStrandFrame);
	...
	*/
	...
	genReadFileFilter.setDescription("XRNA I/O files, old-style XRNA .ss files, old-style XRNA .ps files, or manually edited SVG files");
	...
	/*
	debug("IMG CANVAS is DOUBLE BUFFERED: " + this.getViewImgCanvas().isDoubleBuffered());
	debug("THIS is DOUBLE BUFFERED: " + this.isDoubleBuffered());
	debug("IMG CANVAS is OPTIMIZED: " + this.getViewImgCanvas().isOptimizedDrawingEnabled());
	...
	*/
	...
	/*
	KeyListener keyListeners[] = genImgScrollPane.getVerticalScrollBar().getKeyListeners();
	debug("LISTEN COUNT: " + keyListeners.length);
	*/
	/*
	ActionListener actionListener = genImgScrollPane.getVerticalScrollBar().getActionForKeyStroke(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_UP, 0));
	debug("actionListener: " + actionListener);
	*/
	/*
	genImgScrollPane.getVerticalScrollBar().unregisterKeyboardAction(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_UP, 0));
	*/
...
// InputMap im = genImgScrollPane.getInputMap();
// ActionMap am = genImgScrollPane.getActionMap();
debug("am.size: " + am.size());
debug("im.size: " + im.size());
KeyStroke up = KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0);
Object upKey = im.get(up);
am.put(upKey, null);
*/
	...
	/*
	this.getViewImgCanvas().addKeyListener(new KeyAdapter()
	{
		public void
		keyPressed(KeyEvent keyEvent)
		{
	...
	*/
	/*
	JScrollBar vsb = genImgScrollPane.getVerticalScrollBar();
	JScrollBar hsb = genImgScrollPane.getHorizontalScrollBar();
	vsb.setValue(300);
	hsb.setValue(300);
	*/
	...
	if (complexTabbedPane.getSelectedIndex() == 0)
	{
		...
		ioTabPanel.controlPanel.add(BorderLayout.SOUTH,
			this.complexPickPanel);
		ioTabPanel.controlPanel.add(BorderLayout.NORTH,
			this.scale_Panel);
	}
	else if (complexTabbedPane.getSelectedIndex() == 1)
	{
		annotateTabPanel.add(BorderLayout.CENTER, genImgScrollPane);
		annotateTabPanel.controlPanel.add(BorderLayout.SOUTH,
			this.complexPickPanel);
		annotateTabPanel.controlPanel.add(BorderLayout.NORTH,
			this.scale_Panel);
	}
	else if (complexTabbedPane.getSelectedIndex() == 2) // Edit Tab
	{
		editTabPanel.add(BorderLayout.CENTER, genImgScrollPane);
		editTabPanel.controlPanel.add(BorderLayout.SOUTH,
			this.complexPickPanel);
	...
	else if (complexTabbedPane.getSelectedIndex() == 3) // Format Tab
	{
		...
		formatSStrTabPanel.controlPanel.add(BorderLayout.SOUTH,
			this.complexPickPanel);
	...
	*/
	...
	userConsoleScrollPane = new JScrollPane(
		JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	complexTabbedPane.getModel().addChangeListener(
		new ChangeListener()
		{
			public void
...
throws Exception
{
	...
	ComplexSceneView.this.getComplexScene().
		clearLocationFromHashtable(getCurrentUndoLevel());
	System.gc();
...
throws Exception
{
	...
	ComplexSceneView.this.getComplexScene().
		clearBasePairFromHashtable(getCurrentUndoLevel());
	System.gc();
...
runTestBt()
{
	try
	{
		debug("Java memory in use = " +
			(Runtime.getRuntime().totalMemory() -
				Runtime.getRuntime().freeMemory()));

		/*
		printConsole("CURR WIN CENTER: " +
			(this.getWindowViewX() + this.getWindowViewW()/2) + " " +
			(this.getWindowViewY() + this.getWindowViewH()/2));
		*/
		...
		String outFileName = JAIUtil.printImage(this.getOffScreenImg(), genImgOutFile);

		genImgOutFile = new File("images/test_on_img.jpg");
		debug("WORKING ON: " + genImgOutFile.toString() + " " +
			genImgOutFile.canWrite());
		outFileName = JAIUtil.printImage(this.getSceneImg(), genImgOutFile);
		*/
		...
		*/
		...
		{ 
			handleEx("Exception in ComplexSceneView.runTestBt0:", pe, 101);
		}
		catch (IOException ie)
		{ 
			handleEx("Exception in ComplexSceneView.runTestBt1:", ie, 101);
		}
		*/
	}
	catch (Exception e)
...
public void
runInitFromProperties(File dataFile, String path)
{
	try
	{
		...
		if (fileName.endsWith(".ss") || fileName.endsWith(".xrna") ||
			fileName.endsWith(".xml") || fileName.endsWith(".ps") ||
...
public void
runInitFromProperties(File dataFile)
{
...
public boolean runWriteTRBt() throws Exception { return this.runWriteTRBt(complexSceneOutFile); }

public boolean runWriteSVGBt() throws Exception { return this.runWriteSVGBt(complexSceneOutFile); }

public boolean
runWriteXMLBt(String fileName)
throws Exception
{
...
public boolean
runWriteCSVBt(String fileName) 
throws Exception
{
	return (this.runWriteCSVBt(new File(fileName)));
}

public boolean runWriteTBt(String fileName) throws Exception { return this.runWriteTRBt(new File(fileName)); }

public boolean
runWriteXMLBt(File outFile)
throws Exception
{
	if (outFile == null)
		return (false);
	...
	{
		alert("Can't write to file ending with .ss: " +
			outFile.getName() + "; Need to change extension\n" +
	...
	{
		alert("Can't write to file ending with .ps: " +
			outFile.getName() + "; Need to change extension\n" +
			"to .xrna in Import/Export Tab with 'Write XRNA I/O file' button");
		return (false);
	}
...
throws Exception
{
	...
	try
	{
		//System.out.println("ComplexSceneView.java this.getComplexScene() is: " + this.getComplexScene());
...
public boolean runWriteTRBt(File outFile) throws Exception {
	...
	try { this.getComplexScene().printComplexTR(outFile); }
...
centerScrollBars()
{
	...
	debug("VIEWPORT EXTENT SIZE 0: " + genImgScrollPane.getViewport().getExtentSize());
	debug("VIEW SIZE 0: " + genImgScrollPane.getViewport().getView().getBounds());
	debug("PANE SIZE 0: " + genImgScrollPane.getBounds());
	*/
	...
	/*
	debug("BEFORE: ");
	debug("SCROLL X MIN: " + scrollBarX.getMinimum());
	debug("SCROLL Y MIN: " + scrollBarY.getMinimum());
	debug("SCROLL X MAx: " + scrollBarX.getMaximum());
	debug("SCROLL Y MAx: " + scrollBarY.getMaximum());
	...
	*/
	...
	// debug("setting scroll bar X,Y MXS: " + currW + " " + scrollBarX.getMaximum() + " " + currH + " " + scrollBarY.getMaximum());
	*/
	...
	*/
	...
	debug("VIEWPORT EXTENT SIZE 1: " + genImgScrollPane.getViewport().getExtentSize());
	debug("VIEW SIZE 1: " + genImgScrollPane.getViewport().getView().getBounds());
	debug("PANE SIZE 1: " + genImgScrollPane.getBounds());
	*/
}
...
public void
setMouseToNuc(DrawObject drwObj)
throws Exception
{
	if (drwObj == null)
		return;
	...
	Point2D testPt = new Point2D.Double(
		drwObjAffTrans.getTranslateX(),
		drwObjAffTrans.getTranslateY());
	...
	// this.centerWindowToImgCoords((int)newerTestPt.getX(), (int)newerTestPt.getY());
	JScrollBar scrollBarX = genImgScrollPane.getHorizontalScrollBar();
	JScrollBar scrollBarY = genImgScrollPane.getVerticalScrollBar();
	...
	int newMouseX = viewPortStartX + (int)newerTestPt.getX() -
		scrollBarX.getValue();
	int newMouseY = viewPortStartY + (int)newerTestPt.getY() -
		scrollBarY.getValue();
...
public void
setWindowToDrawObjectCenter(DrawObject drwObj)
throws Exception
{
	if (drwObj == null)
		return;
	...
	if ((newMouseX < viewPortStartX) ||
		(newMouseX > viewPortEndX) ||
		(newMouseY < viewPortStartY) ||
		(newMouseY > viewPortEndY))
	{
		...
		newMouseX = viewPortStartX + (int)newerTestPt.getX() -
			scrollBarX.getValue();
		newMouseY = viewPortStartY + (int)newerTestPt.getY() -
			scrollBarY.getValue();
	}
	this.viewRobot.mouseMove(newMouseX, newMouseY);
}
...
throws Exception
{
	this.centerWindowToImgCoords(
		(int)this.viewSpaceX(this.getWindowViewX() + this.getWindowViewW()/2),
		(int)this.viewSpaceY(this.getWindowViewY() + this.getWindowViewH()/2));
}

/* Doesn't work yet (but not sure I'll need it)
public void
newCenterWindowToImgCoords(int imgX, int imgY)
throws Exception
{
	if (!this.isShowing())
		return;
	...
	// debug("viewport.getViewSize(): " + viewport.getViewSize());
	ViewImgCanvas view = (ViewImgCanvas)viewport.getView();
	debug("ViewImgCanvas.size: " + view.getWidth() + " " + view.getHeight());

	viewport.scrollRectToVisible(
		new Rectangle(
			imgX - scrollBarX.getVisibleAmount()/2,
...
public void
centerWindowToImgCoords(int imgX, int imgY)
throws Exception
{
	if (!this.isShowing())
		return;
...
//	debug("CENTERING ON IMG X,Y: " + imgX + " " + imgY);
	...
	/*
	debug("SETTING SCROLLX,Y TO: " + (imgX - scrollBarX.getVisibleAmount()/2) + " " + (imgY - scrollBarY.getVisibleAmount()/2));
	*/
	...
	*/
	...
	debug("BUT gets SET TO: " + scrollBarX.getValue() + " " +
		scrollBarY.getValue());
	debug("OR gets SET TO: " + (scrollBarX.getValue() + scrollBarX.getVisibleAmount()/2) + " " +
...
updateImgWindowView()
{
	...
	{
		this.getViewImgCanvas().setPreferredSize(
	...
	JScrollBar scrollBarY =
		this.genImgScrollPane.getVerticalScrollBar();
	JScrollBar scrollBarX =
		this.genImgScrollPane.getHorizontalScrollBar();

	scrollBarX.setMaximum(currW);
	scrollBarY.setMaximum(currH);
	
	this.setWindowViewParams(
		scrollBarX.getValue(), scrollBarY.getValue(),
		scrollBarX.getVisibleAmount(), scrollBarY.getVisibleAmount());
}

class ComplexPickPanelShowing
extends Thread
{
	public void
	run()
...
throws Exception
{
	...
	*/
	...
	for (int sceneItemID = 0;sceneItemID < scene.getItemCount();sceneItemID++)
	{
		...
		ComplexSSDataCollection2D ssDataCollection =
...
public void
setPreShapesList(Vector preShapesList)
{
...
public void
setPostShapesList(Vector postShapesList)
{
...
setComplexScene(ComplexScene2D complexScene)
{
	...
	*/
	/*
	debug("TREE.depth, childcount, leafcount: " +
		node.getDepth() + " " + node.getChildCount() + " " + node.getLeafCount());
	*/
	
	/*
	if (pickStrandBt != null)
	{
		if (node.getLeafCount() > 1)
			pickStrandBt.setVisible(true);
		else
			pickStrandBt.setVisible(false);
		// this.updateUI();
	}
	*/
...
public void runSetFromInputFile() throws Exception {
	...
	if (this.getCurrentInputFile().getName().endsWith(".ss")) {
		...
		newComplex.setNewSSComplexElement(SSData2D.getSSDataFromSSFile(
			getCurrentInputFile()));
		...
		ComplexSSDataCollection2D newComplex = new ComplexSSDataCollection2D(ssName, (String)null);
		try {
			newComplex.setNewSSComplexElement(SSData2D.getSSDataFromPSFile(
				getCurrentInputFile()));
	...
	} else if (this.getCurrentInputFile().getName().endsWith(".xml") || this.getCurrentInputFile().getName().endsWith(".xrna")) {
		this.parseXrna();
	} else if (this.getCurrentInputFile().getName().endsWith(".svg")) {
		new SVGToXRNAParser();
	} else {
		alert("Currently can't work with file: " +
	...
	private class SVGToXRNAParser {
		private HashMap<String, HashMap<String, Object>>
			styleDataMap = new HashMap<String, HashMap<String, Object>>();
		...
		private SVGToXRNAParser() throws Exception {
			File
				temporaryXrna = File.createTempFile("temp", ".xrna");//new File("C:\\Users\\caede\\Desktop\\temp.xrna");
			...
			try (BufferedReader reader = new BufferedReader(new FileReader(getCurrentInputFile())); PrintWriter writer = new PrintWriter(new FileWriter(temporaryXrna))) {
				...
				for (; _line != null; _line = reader.readLine()) {
					...
					} else if (lineToLowerCase.contains(nucleotideLinePrefix) || lineToLowerCase.contains(labelLinePrefix) || lineToLowerCase.contains(lettersLinePrefix) || lineToLowerCase.contains(sequenceLinePrefix) || lineToLowerCase.contains(helixLinePrefix) || lineToLowerCase.contains(labelsLinePrefix)) {
				...
				if (nonstandardSVGFormat) {
					...
					for (String line : lines) {
						...
						font = new Font(fontFamily, Font.PLAIN, (int)Math.round(fontSize));
					...
					for (String line : lines) {
						if (line.contains("class=\"numbering-line\"")) {
							...
							labelLines.add(new Line2D.Float(x1, y1, x2, y2));
							...
							float
								...
								x2 = Float.parseFloat(this.boundedSubstring(line, "x2=\"", "\"")),
								y2 = Float.parseFloat(this.boundedSubstring(line, "y2=\"", "\""));
							nucleotideLines.add(new Line2D.Float(x1, y1, x2, y2));
							...
							labels.add(new Tuple4<>(label, new Rectangle2D.Float(x - width / 2f, y - fontSize, width, fontSize), font, null));
							...
							if (letter.equalsIgnoreCase("A") || letter.equalsIgnoreCase("C") || letter.equalsIgnoreCase("G") || letter.equalsIgnoreCase("U")) {
								letters.add(new Tuple4<>(letter, new Rectangle2D.Float(x, y - fontSize, metrics.stringWidth(letter), fontSize), font, null));
							}
							...
							String
								letter = this.boundedSubstring(line, ">", "<").strip();
							if (letter.equalsIgnoreCase("A") || letter.equalsIgnoreCase("C") || letter.equalsIgnoreCase("G") || letter.equalsIgnoreCase("U")) {
								letters.add(new Tuple4<>(letter, new Rectangle2D.Float(x, y - fontSize, metrics.stringWidth(letter), fontSize), font, null));
							}
						}
					...
					float
						dx = 0f,
						dy = 0f;
					for (Tuple4<String, Rectangle2D.Float, Font, Float> letter : letters) {
						dx += letter.t1.width;
						dy -= letter.t1.height;
					}
					dx = dx * 1.2f / (letters.size() * 2f);
					dy /= (letters.size() * 2f);
					
					for (Line2D.Float line : labelLines) {
						line.x1 += dx;
						line.x2 += dx;
						line.y1 += dy;
						line.y2 += dy;
					...
					for (Tuple4<String, Rectangle2D.Float, Font, Float> label : labels) {
						label.t1.x += label.t1.width / 6f;
					}
				} else {
					for (String line : styleLines) {
						int
							dotIndex = line.indexOf("."),
							openBracketIndex = line.indexOf("{");
						String
							styleName = line.substring(dotIndex + 1, openBracketIndex);
						HashMap<String, Object>
							styleData = new HashMap<>();
						for (String styleDatum : line.substring(openBracketIndex + 1, line.indexOf("}")).split(";")) {
							int
							...
							switch (datumLabel) {
								case "fill":
								case "stroke":
									datum = datumContent.trim().equalsIgnoreCase("none") ? new Color(0f, 0f, 0f, 1f) : new Color(Integer.parseInt(datumContent.substring(1), 16), true);
									break;
								case "font-family":
						...
						styleDataMap.put(styleName, styleData);
					}
					boolean
					...
					do {
						String
							lineToLowerCase = _line.toLowerCase();
						if (lineToLowerCase.contains(nucleotideLinePrefix)) {
							this.parseLines(reader, nucleotideLines);
						} else if (lineToLowerCase.contains(labelLinePrefix)) {
							this.parseLines(reader, labelLines);
						} else if (lineToLowerCase.contains(sequenceLinePrefix)) {
							this.parseText(reader, letters);
							formatRequiresXYShift = true;
						} else if (lineToLowerCase.contains(lettersLinePrefix)) {
							this.parseText(reader, letters);
						} else if (lineToLowerCase.contains(helixLinePrefix)) {
							this.parseText(reader, helixLabels);
						} else if (lineToLowerCase.contains(labelsLinePrefix)) {
							this.parseText(reader, labels);
						}
						_line = reader.readLine();
					} while (_line != null);
				...
				for (Line2D.Float line : labelLines) {
					Tuple4<String, Rectangle2D.Float, Font, Float>
						label = this.findPairLabel(line);
					if (label != null) {
						// Label Content | BoundingBox | Font info | Stroke Width | Nuc ID
						Vector2
							v1 = new Vector2(line.x1, line.y1);
						if (Vector2.dot(Vector2.subtract(new Vector2(line.x2, line.y2), v1), Vector2.subtract(new Vector2((float)label.t1.getCenterX(), (float)label.t1.getCenterY()), v1)) < 0) {
							line = new Line2D.Float(line.getP2(), line.getP1());
						}
						Tuple5<String, Rectangle2D.Float, Font, Float, Integer>
							letter = this.findPairLetter(line);
						if (letter != null) {
							pairsData.add(new Tuple3<>(label, letter, line));
						}
					}
				}
				String
				...
				Vector2
					mid = new Vector2((minX + maxX) / 2f, (minY + maxY) / 2f);
				for (Tuple4<String, Rectangle2D.Float, Font, Float> letter : letters) {
					Vector2
						letterLocation = Vector2.subtract(new Vector2(letter.t1.x, letter.t1.y), mid);
					letterLocation.y = -letterLocation.y;
					writer.println(letter.t0 + " " + letterLocation.x + " " + letterLocation.y);
				}
				
				writer.println("</NucListData>");
				String
					lineStarter = "<Nuc RefIDs='1-" + letters.size() + "'";
				writer.println(lineStarter + " Color='ff0000' FontID='0' FontSize='" + 4 + "'/>");
				...
				for (Tuple3<Tuple4<String, Rectangle2D.Float, Font, Float>, Tuple5<String, Rectangle2D.Float, Font, Float, Integer>, Line2D.Float> pair : pairsData) {
					double
					...
					double
						...
						dx = (pair.t2.x1 - centerX),
						dy = -(pair.t2.y1 - centerY);
					writer.println("l " + dx + " " + dy + " " + (pair.t2.x2 - centerX) + " " + -(pair.t2.y2 - centerY) + " " + strokeWidth + " " + 255 + " 0.0 0 0 0 0");
					writer.println("s " + (pair.t0.t1.getCenterX() - pair.t1.t1.getCenterX()) + " " + (pair.t1.t1.getCenterY() - pair.t0.t1.getCenterY()) + " 0.0 " + pair.t0.t2.getSize() + " 0 " + 255 + " \"" + pair.t0.t0 + "\"");
					writer.println("</LabelList>");
					writer.println("</Nuc>");
				}
				...
				float
					...
					maximumDistance = 0f;
				for (Line2D.Float nucleotideLine : nucleotideLines) {
					float
				...
				double
					...
					interpolatedDistance = (Math.exp(logMinimumDistance + (logMaximumDistance - logMinimumDistance) * 0.75f));
				for (Line2D.Float nucleotideLine : nucleotideLines) {
					float
						distanceScalar = (float)interpolatedDistance / (this.distance(nucleotideLine) * 2f);
					nucleotideLine = this.scaledLine(nucleotideLine, distanceScalar, distanceScalar);
					
					Tuple5<String, Rectangle2D.Float, Font, Float, Integer>
						letter0 = this.findClosestLetter(this.scaledLine(nucleotideLine, 0f, lineExtensionScalar)),
						letter1 = this.findClosestLetter(this.scaledLine(nucleotideLine, lineExtensionScalar, 0f));
					if (letter0.t4.intValue() != letter1.t4.intValue()) {
						writer.println("<BasePairs nucID='" + letter0.t4 + "' length='1' bpNucID='" + letter1.t4 + "' />");
					} else {
						debugLines.add(new Tuple2<>(this.scaledLine(nucleotideLine, 0f, lineExtensionScalar), Color.ORANGE));
						debugLines.add(new Tuple2<>(this.scaledLine(nucleotideLine, lineExtensionScalar, 0f), Color.MAGENTA));
//						throw new ArithmeticException("Geometry mistake; nucleotide algorithmically found to be bound to itself.");
					}
				}
				...
				writer.close();
				setCurrentInputFile(temporaryXrna);
				parseXrna();
			}
			JFrame
				frame = new JFrame();
...
//							g2D.draw(modify(label.t1));
						...
						for (Tuple4<String, Rectangle2D.Float, Font, Float> helixLabel : helixLabels) {
							if (helixLabel.t3 != null) {
								g2D.setStroke(new BasicStroke(helixLabel.t3));
							}
							g2D.setColor(Color.RED);
							g2D.draw(modifyLabelBounds(helixLabel.t1));
							g2D.setColor(Color.WHITE);
						...
						for (Line2D.Float line : nucleotideLines) {
							g2D.draw(line);
						}
						...
						for (Tuple2<Line2D.Float, Color> line : debugLines) {
							g2D.setColor(line.t1);
							g2D.draw(line.t0);
						}
						
						Random
							random = new Random(0);
						for (Tuple3<Tuple4<String, Rectangle2D.Float, Font, Float>, Tuple5<String, Rectangle2D.Float, Font, Float, Integer>, Line2D.Float> pairDatum : pairsData) {
							...
							g2D.draw(modifyLabelBounds(pairDatum.t0.t1));
							
							if (pairDatum.t1.t3 != null) {
								g2D.setStroke(new BasicStroke(pairDatum.t0.t3));
							}
							Rectangle2D.Float
								modifiedT1 = modifyLetterBounds(pairDatum.t1.t1);
							float
								maximumAxis = Math.max(modifiedT1.width, modifiedT1.height);
			...
			frame.addMouseMotionListener(new MouseMotionAdapter() {
				...
				public void mouseDragged(MouseEvent event) {
					Point2D
						p2D = event.getLocationOnScreen();
					origin.x += (p2D.getX() - mouse.x) / scale;
					origin.y += (p2D.getY() - mouse.y) / scale;
					mouse.x = (float)p2D.getX();
					mouse.y = (float)p2D.getY();
					frame.repaint();
				}
...
//			frame.setVisible(true);
		...
		private Tuple4<String, Rectangle2D.Float, Font, Float> findPairLabel(Line2D.Float line) {
			// Attempt to find a direct, full collision between the line and the bounds
			for (Tuple4<String, Rectangle2D.Float, Font, Float> label : labels) {
				Rectangle2D.Float
			...
			// Attempt to find a direct, full collision between the scaled-up line and the bounds
			for (Tuple4<String, Rectangle2D.Float, Font, Float> label : labels) {
				Rectangle2D.Float
				...
				float
					...
					y1 = t1.y + t1.height;
				Line2D.Float
					scaledLine = this.scaledLine(line, lineExtensionScalar, lineExtensionScalar),
					edge0 = new Line2D.Float(x0, y0, x1, y0),
					edge1 = new Line2D.Float(x1, y0, x1, y1),
					edge2 = new Line2D.Float(x1, y1, x0, y1),
					edge3 = new Line2D.Float(x0, y1, x0, y0);
			...
			// Attempt to find the bounds with the nearest vertex to the line; nearest vertex must be within a maximum-distance threshold.
			double
			...
			for (Tuple4<String, Rectangle2D.Float, Font, Float> label : labels) {
				...
				float
					...
					y1 = t1.y + t1.height;
				double
					currentMinimumDistanceSquared = Math.min(Math.min(Math.min(line.ptSegDistSq(x0, y0), line.ptSegDistSq(x1, y0)), line.ptSegDistSq(x1, y1)), line.ptSegDistSq(x0, y1));
				if (currentMinimumDistanceSquared < minimumDistanceSquared) {
					minimumDistanceSquared = currentMinimumDistanceSquared;
					minimumDistanceLabel = label;
				}
			}
			return minimumDistanceSquared / new Point2D.Float(line.x1, line.y1).distanceSq(new Point2D.Float(line.x2, line.y2)) < 15f ? minimumDistanceLabel : null;
		}
...
//				float
//					maximumRadius = Math.max(letter.t1.width, letter.t1.height) / 2f;
//				Rectangle2D.Float
//					modifiedT1 = this.modifyLetterBounds(letter.t1);
//				if (line.ptSegDist(modifiedT1.getCenterX(), modifiedT1.getCenterY()) <= maximumRadius) {
//					return new Tuple5<>(letter.t0, letter.t1, letter.t2, letter.t3, index + 1);
//				}
//			}
//			return findClosestLetter(this.scaledLine(line, lineExtensionScalar, lineExtensionScalar));
		}
		
		private Tuple5<String, Rectangle2D.Float, Font, Float, Integer> findMostDirectLetter(Line2D.Float line) {
			Vector2
				v1 = new Vector2(line.x1, line.y1),
				v2 = new Vector2(line.x2, line.y2),
				normalizedLineDv = Vector2.normalize(Vector2.subtract(v2, v1)),
				centerV1V2 = Vector2.scaleDown(Vector2.add(v1, v2), 2f);
			Tuple4<String, Rectangle2D.Float, Font, Float>
				mostDirectLetter = null;
			int
				mostDirectIndex = -1;
			float
				mostDirectDotProduct = -1f;
			for (int index = 0; index < letters.size(); index++) {
				Tuple4<String, Rectangle2D.Float, Font, Float>
					letter = letters.get(index);
				Vector2
					normalizedDv = Vector2.normalize(Vector2.subtract(new Vector2((float)letter.t1.getCenterX(), (float)letter.t1.getCenterY()), centerV1V2));
				float
					dotProduct = Vector2.dot(normalizedLineDv, normalizedDv);
				if (dotProduct > mostDirectDotProduct) {
					mostDirectLetter = letter;
					mostDirectIndex = index;
					mostDirectDotProduct = dotProduct;
				}
			}
			return mostDirectLetter == null ? null : new Tuple5<String, Rectangle2D.Float, Font, Float, Integer>(mostDirectLetter.t0, mostDirectLetter.t1, mostDirectLetter.t2, mostDirectLetter.t3, mostDirectIndex);
		}
		...
		private Tuple5<String, Rectangle2D.Float, Font, Float, Integer> findClosestLetter(Line2D.Float line) {
			...
			for (int index = 0; index < letters.size(); index++) {
				...
				if (newDistance < closestDistance) {
					...
					closestPairLetter = new Tuple5<String, Rectangle2D.Float, Font, Float, Integer>(letter.t0, letter.t1, letter.t2, letter.t3, index + 1);
				}
			}
		...
		private Rectangle2D.Float modifyLabelBounds(Rectangle2D.Float rect) {
			return new Rectangle2D.Float(rect.x - rect.width / 20f * 1.25f, rect.y + rect.height / 10f, rect.width, rect.height);
		}
		...
		private Line2D.Float scaledLine(Line2D.Float line, float scalar0, float scalar1) {
			float
				...
				lineX2 = line.x2,
				lineY2 = line.y2,
				xHalf = (lineX1 + lineX2) / 2f,
				yHalf = (lineY1 + lineY2) / 2f;
		...
		private void mouseWheelMoved(double deltaWheel) {
			this.scale *= (float)Math.pow(deltaScalar, deltaWheel);
		...
		private void parseText(BufferedReader reader, ArrayList<Tuple4<String, Rectangle2D.Float, Font, Float>> labels) throws IOException {
			for (String line = reader.readLine(); line != null && !line.contains("</g>"); line = reader.readLine()) {
				...
				HashMap<String, Object>
					classAggregate = new HashMap<String, Object>();
				for (String _class : this.boundedSubstring(line, "class=\"", "\"").split(" ")) {
					HashMap<String, Object>
						classData = styleDataMap.get(_class);
					try {
						...
						int
							index = 0;
						for (String key : styleDataMap.keySet()) {
				...
				FontMetrics
					metrics = ComplexParentFrame.frame.getFontMetrics(font);
				labels.add(new Tuple4<String, Rectangle2D.Float, Font, Float>(text, new Rectangle2D.Float(position.x, position.y - fontSize, metrics.stringWidth(text), fontSize), font, (Float)classAggregate.get("stroke-width")));
			}
		}
		...
		private String boundedSubstring(String string, String query, String suffix) {
			...
			try {
				...
				System.out.println(string + "\t" + query + "\t" + suffix);
				throw ex;
			}
		}
	...
	private void parseXrna() throws Exception {
		if (this.getCurrentInputFile().exists() && !this.getCurrentInputFile().canWrite()) {
			if (!(runTestOnly || suppressAlert)) {
				alert("Warning: " +
					this.getCurrentInputFile().getName() +
...
Parameters:
path - the TreePath identifying a node of javax.swing.JTree. 
The trick is to compute the TreePaths. The general algorithm
would be to compute the leaf nodes and force expansion of each
of the leaf nodes unique parent. 

If your tree is made out of a DefaultMutableTreeNodes then you
can get the enumeration of all the nodes and thus compute the
TreePaths easily. 
*/

public boolean
sceneContainsOneRNAStrand()
{
	DefaultMutableTreeNode node = (DefaultMutableTreeNode)this.getComplexScene().createComplexNodes();
...
public void
runPDFPrinterJob(boolean showDialog, String jobName)
throws Exception
{
	/* SAVE if want to go back to using JPDFPrinter.jar from QOPPA 
	PrinterJob pj = PDFPrinterJob.getPrinterJob();
	...
	if (fileName != null)
	{
		((PDFPrinterJob)pj).print(fileName);
	}
	else if (outStream != null)
	{
		((PDFPrinterJob)pj).print(outStream);
	}
...
public void
runComplexPrinterJob(boolean showDialog, String jobName)
{
try {
...
public void
runComplexPDFJob(boolean showDialog, String jobName)
{
try {
    String cn = UIManager.getSystemLookAndFeelClassName();
    UIManager.setLookAndFeel(cn); // Use the native L&F
...
public void
setFindNucSSData(SSData2D findNucSSData)
{
	...
	if (this.getComplexScene() != null)
	{
		...
		else
		{
			...
			for (Enumeration e = rootNode.depthFirstEnumeration();e.hasMoreElements();)
			{
				...
				ComplexScene parentComplexCollection =
					this.getComplexScene().getChildByName(node.getParent().toString());
				...
				sstr_RB.addActionListener(new ActionListener()
				{
					...
					actionPerformed(ActionEvent evt)
					{
						try
						{
							SSData2D sstr = (SSData2D)getComplexScene().getChildByName(
...
throws Exception
{
			...
			public void internalFrameClosed(InternalFrameEvent e)
			{
				try
				{
					InternalFrameListener[] listenerList =
						findFrame.getInternalFrameListeners();
	...
	updateBasicInternalFrame("Find Structures",
		"enter nuc number in text field:",
		findPanel, findFrame);
	if (findFrame.isIcon())
...
public void
setWindowLocationFromFindString(String nucString)
{
	int nucID = 0;
	try
	{
...
public void
setWindowLocationFromFindString(int nucID)
{
	if (this.getComplexScene() == null)
		return;
	...
	if (nuc == null)
	{
		alert("Non-existent nuc in SSData " + sstr.getName() +
			"at: " + nucID);
		return;
	}
...
public void
setUserConsoleString(String userConsoleString)
{
    this.userConsoleString = userConsoleString;
	propertyChangeListeners.firePropertyChange("CurrentUserConsoleString",
		null, userConsoleString);
}
...
setCurrentComplexPickMode(int currentComplexPickMode)
{
	...
	propertyChangeListeners.firePropertyChange("CurrentComplexPickMode",
		new Integer(oldCurrentComplexPickMode), new Integer(currentComplexPickMode));
}
...
public void
setComplexXMLRepositoryPath(String complexXMLRepositoryPath)
{
    this.complexXMLRepositoryPath = complexXMLRepositoryPath;
}

public String
getComplexXMLRepositoryPath()
{
...
public void
setLandscapeView(boolean landscapeMode, boolean redraw)
throws Exception
{
	if (landscapeMode)
	{
...
throws Exception
{
	...
	DefaultMutableTreeNode node =
		(DefaultMutableTreeNode)this.pickStrandTree.
	...
	switch (node.getLevel())
	{
	  ...
	  case 1 : // ssdata collection level
		complexCollection =
			this.getComplexScene().getChildByName(node.toString());
	  ...
	  case 2 : // ssdata level
		...
		ComplexScene parentComplexCollection =
			this.getComplexScene().getChildByName(parentNode.toString());
		if (parentComplexCollection == null)
			throw new Exception("Error in " +
				"ComplexSceneView.getComplexTreePick(): " +
				"Invalid node: " + node);
		...
		complexCollection =
			parentComplexCollection.getChildByName(node.toString());
		if (complexCollection == null)
			throw new Exception("Error in " +
				"ComplexSceneView.getComplexTreePick(): " +
				"Invalid node: " + node);
	  ...
	  default :
		throw new Exception("Error in " +
			"ComplexSceneView.getComplexTreePick(): Invalid Level: " +
			node.getLevel());
	}
...
resetFigureScale(double val)
{
	...
	{
		/*
		propertyChangeListeners.firePropertyChange("CurrentScaleValue",
			null, new Double(val));
		*/
		scaleTF.setText(Double.toString(val));
...
public void
setComplexPickScaleVal(double complexPickScaleVal)
{
	this.complexPickScaleVal = complexPickScaleVal;
}

public double
getComplexPickScaleVal()
{
...
public void
setCurrentUndoLevel(int currentUndoLevel)
{
...
public void
resetCurrentUndoLevel()
{
	this.setCurrentUndoLevel(0);
}

public void
splitOffSStr(NucCollection2D nucCollection)
throws Exception
{
	if (nucCollection == null)
		throw new ComplexException(
			ComplexDefines.COMPLEX_SCENE_SSTR_SPLIT_ERROR_MSG,
		...
		while (true)
		{
			...
			if (nuc.getGroupName() == null)
				throw new ComplexException(
					ComplexDefines.COMPLEX_SCENE_SSTR_SPLIT_ERROR_MSG,
					ComplexDefines.COMPLEX_SCENE_SSTR_SPLIT_ERROR,
					"nuc: " + nuc.getID() + " in " + sstr.getName() + " " +
					"isn't in a named group");

			// for now no basepairing allowed across named groups
			if (nuc.isFivePrimeBasePair() && (
				((nuc.getBasePair().getGroupName() == null) || !nuc.getBasePair().getGroupName().equals(nuc.getGroupName()))))
				throw new ComplexException(
					ComplexDefines.COMPLEX_SCENE_SSTR_SPLIT_ERROR_MSG,
					ComplexDefines.COMPLEX_SCENE_SSTR_SPLIT_ERROR,
					"nuc: " + nuc.getID() + " in " + sstr.getName() + " " +
					"basepairs outside of named group");
...
private void
handleEx(String extraMsg, Throwable t, int id)
{
	if (runTestOnly)
	{
		switch(id)
	...
	else
	{
		switch(id)
		{
		  /*
		  case 98 :
			showUserMsg("ComplexXMLParser Error:\n" + extraMsg + " " +
				t.toString() + "\n");
			break;
		  */
		  default :
